...
#     ])
    ...
    if args.labeled_batch_size:
        batch_sampler = data.TwoStreamBatchSampler(
            unlabeled_idxs, labeled_idxs, args.batch_size, args.labeled_batch_size) # Unlabeled and labeled data
    else:
...
def train(train_loader, model, ema_model, optimizer, epoch):
    ...
    for i, ((input, ema_input), target) in enumerate(train_loader):
        ...
        # Cross Entropy Loss for labeled data between model(x) amd trie y
        ...
        if not args.supervised_mode:
            ...
            # KL loss for unlabeled data, KL between ema_model(x) and ema_model(x+r_adv)
            v_loss = losses.vat_loss(ema_model, ema_input_var, ema_model_out)
            ...
            if args.consistency:
                ...
                # MSE loss for unlabeld data between model(x) and ema_model(x)
                consistency_weight = get_current_consistency_weight(epoch)
                consistency_loss = consistency_weight * consistency_criterion(model_out, ema_logit) / minibatch_size
            else:
                consistency_loss = 0
            ...
            # new loss function
            loss = class_loss + consistency_loss + v_loss
        else:
            loss = class_loss
